...
import { getContent, getPageBySlug } from '@/v2/data';
...
const HeroSection = () => {
  ...
  const heroTitle = homePage?.title === 'Home' && !homePage?.hero_title
    ? "Escape to Nature's Embrace"
    : (homePage?.hero_title || "Escape to Nature's Embrace");
  const heroSubtitle = homePage?.subtitle || "A mindful retreat in Matheran's no-car eco zone";

  const slides: Array<{ type: string; content: string; alt: string }> =
    homePage?.hero_gallery && Array.isArray(homePage.hero_gallery) && homePage.hero_gallery.length > 0
      ? homePage.hero_gallery.map((url: string, index: number) => ({
          type: 'image',
          content: url,
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 md:p-8 border border-white/20 max-w-4xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 items-end">
                <div className="space-y-2">
                  <label className="text-white/80 text-sm font-medium block text-left">{getContent('hero.checkIn', 'Check-in')}</label>
                ...
                <div className="space-y-2">
                  <label className="text-white/80 text-sm font-medium block text-left">{getContent('hero.checkOut', 'Check-out')}</label>
                ...
                <div className="space-y-2">
                  <label className="text-white/80 text-sm font-medium block text-left">{getContent('hero.guests', 'Guests')}</label>
                  <GuestSelector
                    totalGuests={guests}
                ...
                >
                  {getContent('hero.exploreStay', 'Explore Stay')}
