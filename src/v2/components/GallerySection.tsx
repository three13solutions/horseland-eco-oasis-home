...
import { galleryImages } from '@/v2/data';
...
const GallerySection = () => {
  ...
  const officialPhotos = galleryImages.filter(
    (g) => g.media_type === 'image' && g.source_type !== 'guest_upload' && g.source_type !== 'ugc'
  );
  const ugcPhotos = galleryImages.filter(
    (g) => g.media_type === 'image' && (g.source_type === 'guest_upload' || g.source_type === 'ugc')
  );
