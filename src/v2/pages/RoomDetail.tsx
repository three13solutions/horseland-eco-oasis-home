...
import { getRoomTypeById, roomUnits } from '@/v2/data';
...
const RoomDetail = () => {
  ...
  const roomData: any = useMemo(() => getRoomTypeById(roomId), [roomId]);
  const [showSeasonalPricing, setShowSeasonalPricing] = useState(false);
  // Seasonal rate calendar (seasons/day-types) is not part of the static content
  // layer yet; the room's own `seasonal_pricing` field (if any) is still shown below.
  const seasonalPricing: any[] = [];
  const seasons: any[] = [];
  ...
  // Static "availability" — count active room units for this room type.
  const availableUnits = useMemo(() => {
    if (!roomData?.id) return 0;
    return roomUnits.filter((u) => u.room_type_id === roomData.id && u.is_active !== false && u.status !== 'maintenance').length;
  }, [roomData]);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/v2/stay')}>Back to Rooms</Button>
        </div>
      </div>
    );
  }
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-24">
                <CardContent className="p-6">
                  ...
                  <Button variant="outline" className="w-full mb-6 font-body" onClick={() => navigate('/v2/stay')}>
                    View All Rooms
                  </Button>
