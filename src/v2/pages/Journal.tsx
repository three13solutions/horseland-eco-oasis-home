...
import { publishedBlogPosts, getPageBySlug } from '@/v2/data';
...
const Journal = () => {
  ...
  const page = getPageBySlug('journal');
  ...
  const heroTitle = page?.title || 'Mountain Stories & Insights';
  const heroSubtitle = page?.subtitle || 'Tales from the hills, wellness wisdom, and travel inspiration';
  ...
  const allPosts = (publishedBlogPosts as unknown as BlogPost[]).filter((p) =>
      ...
      {featuredPost && (
        <section className="py-16">
          <div className="max-w-6xl mx-auto px-4">
            ...
            <Link to={`/v2/journal/${featuredPost.slug}`}>
