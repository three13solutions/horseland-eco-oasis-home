...
import DiningGalleryCollage from '@/v2/components/DiningGalleryCollage';
import { Button } from '@/components/ui/button';
import { Clock, Leaf, Award, UtensilsCrossed, ChefHat, Sparkles } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { getPageBySlug, activeMeals, getMediaUrl } from '@/v2/data';

const Dining = () => {
  const navigate = useNavigate();
  const { toast } = useToast();

  const pageData = getPageBySlug('dining');
  const heroImage = pageData?.hero_image || 'https://images.unsplash.com/photo-1544025162-d76694265947?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80';
  const heroTitle = pageData?.title || 'Farm-to-Table Dining';
  const heroSubtitle = pageData?.subtitle || 'Savor authentic flavors crafted from local ingredients';

  const content = (pageData?.structured_content as any) || {};
  const inRoomDiningImage = getMediaUrl('in-room-dining', 'https://images.unsplash.com/photo-1590846406792-0adc7f938f1d?w=600&h=400&fit=crop');
  const candleLightDinnerImage = content.candleLightDinner?.image || 'https://images.unsplash.com/photo-1470337458703-46ad1756a187?w=600&h=400&fit=crop';
  const chefNotesImage = content.chefNotes?.image || 'https://images.unsplash.com/photo-1556910103-1c02745aae4d?w=600&h=400&fit=crop';
