...
import { getBlogPostBySlug, blogPosts } from '@/v2/data';
...
const BlogPost = () => {
  ...
  const post = getBlogPostBySlug(slug) as unknown as BlogPost | null;

  const relatedPosts = post
    ? ((blogPosts as unknown as BlogPost[])
        .filter((p) => p.category === post.category && (p as any).is_published !== false && p.id !== post.id)
  ...
  const translatedMetaTitle = post.meta_title || post.title;
  const translatedMetaDescription = post.meta_description || post.content.substring(0, 160);
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    ...
    { name: 'Journal', url: `${window.location.origin}/v2/journal` },
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="max-w-4xl mx-auto px-4 pt-8">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Link to="/v2" className="hover:text-foreground">Home</Link>
            <span>/</span>
            <Link to="/v2/journal" className="hover:text-foreground">Journal</Link>
            <span>/</span>
          ...
          <div className="prose prose-lg max-w-none">
            <div 
              ...
              dangerouslySetInnerHTML={{ 
                __html: DOMPurify.sanitize(translatedContent, {
            ...
            <div className="max-w-6xl mx-auto px-4">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {relatedPosts.map((relatedPost) => (
                  <Link
                    key={relatedPost.id}
                    to={`/v2/journal/${relatedPost.slug}`}
