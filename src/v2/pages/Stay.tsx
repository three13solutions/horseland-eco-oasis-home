...
import { Filters } from '@/v2/components/stay/CategoryFilters';
import CategoryCard, { Category } from '@/v2/components/stay/CategoryCard';
import { publishedRoomTypes, getPageBySlug } from '@/v2/data';
...
const Stay = () => {
  const staticCategories = useMemo(() => publishedRoomTypes.map(mapRoomToCategory), []);
  const [categories] = useState<Category[]>(staticCategories);
  const stayPage = getPageBySlug('stay');
  const [heroImage] = useState(stayPage?.hero_image || 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=2000&q=80');
  const [heroTitle] = useState(stayPage?.title || 'Your Mountain Retreat Awaits');
  const [heroSubtitle] = useState(stayPage?.subtitle || 'Choose from our thoughtfully designed rooms and suites');
