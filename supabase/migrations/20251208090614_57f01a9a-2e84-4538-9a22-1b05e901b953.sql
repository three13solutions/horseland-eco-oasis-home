...
-- Drop and recreate with explicit volatility to force recompilation
  ...
  LOOP
    ...
    LOOP
      ...
      LOOP
        ...
        v_day_type := CASE WHEN v_day_of_week IN (0, 5, 6) THEN 'weekend' ELSE 'weekday' END;

        SELECT * INTO v_override FROM tactical_overrides
        WHERE is_active = true AND start_date <= v_date AND end_date >= v_date
        ...
        IF v_override IS NOT NULL THEN
          ...
          IF v_extra_adults > 0 THEN
            SELECT adjustment_value INTO v_extra_adult_charge FROM tactical_overrides
            WHERE is_active = true AND start_date <= v_date AND end_date >= v_date
              AND occupancy_type = 'extra_adult' AND adjustment_value IS NOT NULL
              AND (day_type IS NULL OR day_type = v_day_type)
              AND (min_nights IS NULL OR v_nights >= min_nights)
              AND (
                (room_unit_id IS NOT NULL AND room_unit_id = p_room_unit_id)
            ...
            ORDER BY CASE WHEN room_unit_id IS NOT NULL THEN 1 WHEN room_type_id IS NOT NULL THEN 2 ELSE 3 END,
              min_nights DESC NULLS LAST, created_at DESC LIMIT 1;
            v_daily_room_rate := v_daily_room_rate + COALESCE(v_extra_adult_charge, 0) * v_extra_adults;
          END IF;
          
          IF v_extra_children > 0 THEN
            SELECT adjustment_value INTO v_extra_child_charge FROM tactical_overrides
            WHERE is_active = true AND start_date <= v_date AND end_date >= v_date
              AND occupancy_type = 'extra_child' AND adjustment_value IS NOT NULL
              AND (day_type IS NULL OR day_type = v_day_type)
              AND (min_nights IS NULL OR v_nights >= min_nights)
              AND (
                (room_unit_id IS NOT NULL AND room_unit_id = p_room_unit_id)
          ...
          END IF;
        ELSE
          SELECT final_price INTO v_daily_room_rate FROM calculate_dynamic_price(
      ...
      v_total_meal_cost := 0;
      v_meal_breakdown := '[]'::JSONB;
      v_policy_adjustment := CASE 
        WHEN v_cancellation_policy.adjustment_type = 'percentage' THEN v_total_room_cost * (v_cancellation_policy.adjustment_value / 100)
        WHEN v_cancellation_policy.adjustment_type = 'fixed' THEN v_cancellation_policy.adjustment_value
        ELSE 0 END;
      ...
      v_variants := v_variants || jsonb_build_object(
        'meal_plan_code', v_meal_plan.plan_code, 'meal_plan_name', v_meal_plan.plan_name,
        'meal_plan_description', v_meal_plan.description, 'included_meals', v_meal_plan.included_meal_types,
        'meal_breakdown', v_meal_breakdown, 'cancellation_policy_code', v_cancellation_policy.policy_code,
        'cancellation_policy_name', v_cancellation_policy.policy_name, 'cancellation_terms', v_cancellation_policy.cancellation_terms,
        'room_rate', v_total_room_cost, 'meal_cost', v_total_meal_cost, 'policy_adjustment', v_policy_adjustment,
        'total_price', v_total_price, 'price_per_night', v_price_per_night,
