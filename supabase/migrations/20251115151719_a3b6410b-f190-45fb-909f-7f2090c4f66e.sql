-- Fix parameter name in calculate_rate_variants function
DROP FUNCTION IF EXISTS calculate_rate_variants(uuid, date, date, uuid, integer, text);
  ...
  LOOP
    ...
    LOOP
      ...
      ELSE
        ...
        -- Calculate room rates for each night using CORRECT parameter name p_date
        FOR v_date IN 
          SELECT generate_series(p_check_in, p_check_out - 1, '1 day'::interval)::DATE
        LOOP
          -- FIX: Use p_date instead of p_check_date
          ...
          FROM calculate_dynamic_price(
            ...
            p_date := v_date,  -- FIXED: was p_check_date
        ...
        -- Calculate meal costs (simplified - you may need to implement calculate_meal_cost_breakdown)
        v_total_meal_cost := 0;
        v_meal_breakdown := '[]'::JSONB;
