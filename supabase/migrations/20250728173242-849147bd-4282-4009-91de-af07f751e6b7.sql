-- Update the check constraint to include hotel and guests categories
ALTER TABLE public.gallery_images 
DROP CONSTRAINT gallery_images_category_check;

ALTER TABLE public.gallery_images 
ADD CONSTRAINT gallery_images_category_check 
CHECK (category = ANY (ARRAY['stay'::text, 'dining'::text, 'spa'::text, 'activities'::text, 'events'::text, 'hotel'::text, 'guests'::text, 'gallery'::text]));

-- Now insert the hardcoded images
DO $$
DECLARE
